...
exports.list = catchAsync(async (req, res) => {
  ...
  res.json({ employees: rows, total: count, page, total_pages: Math.ceil(count / limit) });
});
...
exports.create = catchAsync(async (req, res) => {
  ...
  const user = await User.create({ username, email, password_hash, full_name, phone, role: 'employee', region_id, facility_id });
  await log({ userId: req.user.id, action: 'employee_created', entityType: 'staff', entityId: user.id, metadata: { full_name } });
  res.status(201).json({ employee: { ...user.toJSON(), password_hash: undefined } });
});
...
exports.getOne = catchAsync(async (req, res) => {
  ...
  if (!user || user.role !== 'employee') throw new AppError('Employee not found', 404);
...
exports.update = catchAsync(async (req, res) => {
  ...
  await log({ userId: req.user.id, action: 'employee_updated', entityType: 'staff', entityId: user.id });
  res.json({ employee: { ...user.toJSON(), password_hash: undefined } });
});
...
exports.remove = catchAsync(async (req, res) => {
  ...
  // Block deletion if user has associated assessments to preserve referential integrity
  const assessmentCount = await Assessment.count({ where: { user_id: user.id } });
  if (assessmentCount > 0) {
    throw new AppError('Cannot delete employee with associated assessment records. Deactivate instead.', 400);
  }
  ...
  await log({ userId: req.user.id, action: 'employee_deleted', entityType: 'staff', entityId: user.id });
...
exports.toggleStatus = catchAsync(async (req, res) => {
  ...
  res.json({ message: `Employee ${user.is_active ? 'activated' : 'deactivated'}`, is_active: user.is_active });
});
...
exports.resetPassword = catchAsync(async (req, res) => {
  ...
  await user.save();
  await log({ userId: req.user.id, action: 'employee_password_reset', entityType: 'staff', entityId: user.id });
