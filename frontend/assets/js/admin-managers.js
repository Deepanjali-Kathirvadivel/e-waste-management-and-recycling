(function () {
  ...
  async function loadStaff() {
    try {
      ...
      const data = await res.json();
      staffData = data.managers || [];
      renderStaff();
    } catch (e) {
      document.getElementById('staffCount').textContent = 'Error loading managers';
  ...
  function renderStaff() {
    ...
    if (count) count.textContent = `${staffData.length} total managers`;
      ...
      grid.innerHTML = staffData.map(s => `
        <div class="col-md-3 col-6">
          <div class="staff-card">
            ...
            <div class="mt-2"><a href="manager-details.html?id=${s.id}" class="btn btn-sm btn-outline-green w-100">View Details</a></div>
          </div>
        </div>
      `).join('');
    }
    ...
    if (tbody) {
      tbody.innerHTML = staffData.map(s => `
        <tr>
          <td><a href="manager-details.html?id=${s.id}" class="fw-semibold text-dark text-decoration-none">${s.full_name}</a></td>
          <td>${s.username}</td>
  ...
  window.filterStaffTable = function () {
    ...
    if (tbody) {
      ...
      `).join('') : '<tr><td colspan="8" class="text-center text-muted py-3">No managers match your search</td></tr>';
    }
  };
  ...
  window.editStaff = async function (id) {
    ...
    if (!s) return;
    document.getElementById('staffModalTitle').textContent = 'Edit Manager — ' + s.full_name;
  ...
  window.toggleStaffStatus = async function (id) {
    try {
      const res = await fetch(API_BASE + '/admin/managers/' + id + '/status', { method: 'PATCH', headers });
  ...
  window.openDelete = function (id) {
    document.getElementById('deleteConfirmText').textContent = `Delete manager #${id}?`;
  ...
  document.getElementById('confirmDeleteBtn').addEventListener('click', async function () {
    ...
    try {
      const res = await fetch(API_BASE + '/admin/managers/' + id, { method: 'DELETE', headers });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'API failure');
      showToast('Manager deleted successfully');
      bootstrap.Modal.getInstance(document.getElementById('deleteModal')).hide();
      loadStaff();
    } catch (e) { showToast(e.message || 'Error deleting manager', 'error'); }
  });

  window.openResetPwd = function (id) {
    document.getElementById('resetPwdStaffName').textContent = `Reset password for manager #${id}:`;
  ...
  document.getElementById('confirmResetBtn').addEventListener('click', async function () {
    ...
    try {
      const res = await fetch(API_BASE + '/admin/managers/' + id + '/reset-password', {
        method: 'POST',
        headers,
  ...
  document.getElementById('staffForm').addEventListener('submit', async function (e) {
    ...
    try {
      if (editId) {
        ...
        const res = await fetch(API_BASE + '/admin/managers/' + editId, { method: 'PUT', headers, body: JSON.stringify(body) });
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || 'API failure');
        ...
        const res = await fetch(API_BASE + '/admin/managers', {
          method: 'POST',
          headers,
          body: JSON.stringify({ username, email, password, full_name, phone, role, region_id, facility_id })
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || 'API failure');
      ...
      loadStaff();
    } catch (e) { showToast(e.message || 'Error saving manager', 'error'); }
  });

  document.querySelector('[data-bs-target="#staffModal"]')?.addEventListener('click', function () {
    document.getElementById('staffModalTitle').textContent = 'Add New Manager';
    document.getElementById('editStaffId').value = '';
    document.getElementById('staffForm').reset();
